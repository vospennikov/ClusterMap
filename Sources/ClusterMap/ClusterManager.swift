...
open class ClusterManager {
    ...
    ///
    /// - `.nearCenter`: Represents the position of the `MKAnnotation` nearest to the center. Defaults to the center if
    /// no annotations are available.
    ///
    /// - `.average`: Represents the average position for a cluster of `MKAnnotation` instances.
    ///
    /// - `.first`: Represents the position of the first `MKAnnotation` within a cluster. Defaults to the center if no
    /// annotations are available.
    ///
    /// You can develop your own alignment strategy by implementing the `ClusterAlignmentStrategy` protocol on a
    ...
    /// cluster manager, the value of this property is an empty array.
    ...
    ///
    /// - Parameters:
    /// - annotations: An array of annotation objects. Each object in the array must conform to the MKAnnotation
    /// protocol.
    ...
    /// - completion: A closure to be executed when the reload finishes. The closure has no return value and takes a
    /// single Boolean argument that indicates whether or not the reload actually finished before the completion handler
    /// was called.
    open func reload(mapView: MKMapView, completion: @escaping (Bool) -> Void = { finished in }) {
        ...
        operationQueue.addBlockOperation { [weak self, weak mapView] operation in
            ...
            autoreleasepool {
                let (toAdd, toRemove) = self.clusteredAnnotations(
                    zoomScale: zoomScale,
    ...
    /// It splits the visible map rect into smaller subrects and looks for clusters and visible annotations within those
    /// subrects.
    /// If the `shouldDistributeAnnotationsOnSameCoordinate` property is set to `true`, it will also handle annotations
    /// that have the same coordinate but are not clustered together.
    /// - Parameters:
    ...
    open func clusteredAnnotations(
        zoomScale: Double,
        visibleMapRect: MKMapRect,
        operation: Operation? = nil
    ) -> (toAdd: [MKAnnotation], toRemove: [MKAnnotation]) {
        ...
        dispatchQueue.async(flags: .barrier) {
            for value in hash.values where value.count > 1 {
                for (index, annotation) in value.enumerated() {
                    ...
                    (annotation as? MKPointAnnotation)?.coordinate = annotation.coordinate.coordinate(
